...
#(gli spazi in python non influiscono sul codice a meno che sono si trovino all'interno di una stringa di codice)
...
#ci sono vari types:
# 1)la stringa è un qualsiasi testo compreso tra ""
# 2)integer (numeri interi)
# 3)float (numeri decimali; traduzione: virgola mobile)
# 4)boolean (affermazioni di vero o falso; non hanno traduzione (nati dal matematico Bool))
...
#altri valori, che abbiamo elencato, per le variabili sono gli integer e i float, ossia i numeri interi e decimali (utili a tenere traccia dei dati numerici)
...
#le espressioni viste in precedenza possono sommare solo type dello stesso genere
...
#le f-string sono formate da f e la stringa che contiene delle parentesi graffe che racchiudono l'integer, in questo modo
...
#per rendere il programma più intelligente, bisogna creare decision (decisioni) con varie dichiarazioni (statement)
#una di queste dichiarazioni è l'if statement (con la minuscola), in cui se l'if è vera, il programma esegue le linee di codice ad esso associato; però se l'if è falso, il programma salta le linee do codice
#dopo if vanno i valori true o false, detti condition. infine per chiudere la condition si mettono :
...
#nel caso la condizione if sia falsa, però, invece di creare un altra condizione if contraria alla prima, che sblocca delle linee di codice alternativo, possiamo usare la condizione else
...
elif hour<21:     #possiamo aggiungere quanti elif vogliamo, basta che stiano dopo la condizione if e prima la condizione else
    print("buonasera")
elif hour<24:
    print("buonanotte")
else:     #caso in cui sia la condizione if che le condizioni elif sono false
    print("felis navidal")
#ordine giusto delle condizioni: if    elif     (elif...)     else
print("")       #questa linea serve solo a fare una riga di spazio nella console

#nel caso volessimo aggiungere più condizioni per if o elif, possiamo aggiungere l'operatore 'and' tra le condizioni
#se solo una delle condizioni è falsa, le linee di codice vengono saltate
età = 17
permessi = False
privacy = True
...
#per i cambiamenti nel tempo (over time), le variabili cambiano grazie al self-assignment (autonomia)
likes = 5
likes = likes + 3
print(likes)   ##likes = 8
likes = likes - 1
...
likes -= 2     ##likes = 8
print(likes)
#per la moltiplicazione della variabile per un numero si usa *=
likes *= 3     ##likes = 24
print("i likes sono:")
print(likes)
print("")       #questa linea serve solo a fare una riga di spazio nella console
...
commenti = 8
commenti //= 3    ##commenti = 8 // 3 = 2.66666... => si toglie la parte decimale perché è un integer, quindi il risultato è 2
print("i commenti sono:")
print(commenti)
#per la divisione con la virgola bisogna fare il processo lungo=>   variabile = variabile / numero
nu_nummr = 100
nu_nummr = nu_nummr / 3     #divisione con la virgola
print(nu_nummr)       ##33.33333...
print("")       #questa linea serve solo a fare una riga di spazio nella console
...
#la variabile non definita descrive a che loop è stato eseguita la linea di codice, fungendo da counter
...
#il valore della variabile counter quindi indica quante volte è stato eseguito quel blocco di codice
#se nel valore della funzione range però inseriamo 2 valori, che diventano il primo il valore iniziale e il secondo il valore finale, possiamo definire la variabile counter come i valori compresi tra i 2 valori da range
for a in range(7, 15):
    print(a)
#la variabile non definiti per convenzione è i, che sta per index, però possiamo scegliere qualsiasi nome
...
a_caso_3 = 69
a_caso_4 = True
#si possono fare liste in questo modo
a_caso=["adiè", "si gruoos", 69, True]
print(a_caso)
#i valori della lista (detti elementi) possono essere infiniti, e sono numerati da un indice (index), in cui 0 è il primo, 1 è il secondo, 2 è il terzo, 3 è il quarto
...
#gli instructor si usano si usano scrivendo: nome della variabile + . + instructor + (elemento da aggiungere)
...
print(a_caso)
print(a_caso[3])
#pop si usa per rimuovere un elemento. nelle parentesi va l'indice dell'elemento da rimuovere
a_caso.pop(4)
print(a_caso)
#nel caso volessimo creare una nuova lista partendo dalla lista che abbiamo e selezionando solo alcuni elementi, possiamo scrivere
...
#usiamo len (instructor) per visualizzare il numero di elementi in una lista
print(len(artisti))
#possiamo anche memorizzare 
...
#nelle liste di stringhe il valore max sarebbe quello ordinato alfabeticamente per ultimo (più vicino alla z), invece il valore min sarebbe quello ordinato per primo (più vicino alla a)
...
#le liste possono essere anche di stringhe e di numeri e boolean, e poi vengono scritti nell'ordine dell'espressione, cioè:
...
#nel caso di una variabile con le Stringhe contiene più parole, e si vogliono dividere le varie parole formando una lista, usiamo l'instructor split() (dividere)
...
print(funzione(2))                  #viene visualizzato il valore None perché non abbiamo usato return

def funzione_1(parametro):
    variabile_1_2=parametro+1
    return variabile_1_2
print(funzione_1(2))                #nella console viene visualizzato il valore di return
print("")       #questa linea serve solo a fare una riga di spazio nella console
...
#2)le funzioni che riportano valori con return iniziano con get_
...
#le funzioni possono incorporare anche vari blocchi di codice come l'if statement, detto nested (ossia   CONTINUA DA QUIIII)
